...
<template>
    <div class="max-w-sm rounded overflow-hidden shadow-lg" style="margin: 40px auto;">
        <img class="w-full" :src="charData.image" alt="Sunset in the mountains">
        <div class="px-6 py-4">
            <div class="font-bold text-xl mb-2">{{ charData.name }}</div>
            <h6>Status</h6><span>{{ charData.status }}</span>
            <h6>Type</h6><span>{{ charData.type }}</span>
        </div>
        <ul>
            <li v-for="episode in charData.episode">{{ episode }}</li>
        </ul>
    </div>
</template>
